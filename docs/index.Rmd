...
## Facets
A final important function of ggplot I use fairly frequently is called 'faceting'. This is another way to include a 3rd (or more) variable visually except rather than add a third dimension to an existing plot, we create multiple plots that 'filter' for this third variable. Here's an example:
```{r faceting, warning=FALSE}
df %>% 
  ggplot(., aes(age)) + 
  geom_density()

sample_sizes <- df %>% ## Need this dataframe for labels in the following facets.
  group_by(ethnicity) %>%
...
df %>% 
  ...
  facet_wrap(~ethnicity, nrow = 2, ncol = 2, 
             labeller = labeller(ethnicity = function(x) { ## I use this function to create sample size labels to put on the headers of each facet
                 sample_size <- sample_sizes %>% filter(ethnicity == x) %>% pull(sample_size)
                 paste0(x, " (n=", sample_size, ")")
             }))
```

These are density plots (which I **rarely** use) but they look the nicest for the data we have available so here we are. The first plot we can see that the largest proportion of respondents are aged between 40 and 50 years. When we 'facet' these plots, we can see that this shape holds reasonably well for those who are 'Non-Hispanic', which makes sense considering they make up the majority of the overall sample. For those in the Hispanic group, the sample has more people around the 30 years mark. 